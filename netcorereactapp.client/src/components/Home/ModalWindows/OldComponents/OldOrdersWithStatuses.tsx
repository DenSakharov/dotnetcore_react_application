...
export default function OldOrdersWithStatuses(){
    ...
    //старая логика тестирования функионала взаимодествия между заказами и статусами
    const [orders, setOrders] = useState<OrderModel[]>([]);
    const fetchOrders = async () => {
        try {
            const tokenValue = localStorage.getItem("authToken");
            const response =
                await axios.get<OrderModel[]>(`${config.apiUrl}/orders/getorders`, {
                    headers: {
    ...
    return(
        <div>
            ...
            {view &&
                <div className="old-logic-with-oders-and-statuses">
                    <h1>Заказы</h1>
                    <div>
